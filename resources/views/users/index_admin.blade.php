...
@section('title', 'Data Admin dan Karyawan')
...
<!-- Pencarian -->
<br>
<form class="form-inline mt-1" method="get" action="{{ route('admin.index') }}">
    <div class="input-group">
        <select name="categories" id="categoryFilter" class="form-control border border-info">
            <option value="all">Semua Kategori</option>
            @foreach ($groups as $group)
                @if($group->group_id != 7)
                    <option value="{{ $group->group_id }}" {{ request('categories') == $group->group_id ? 'selected' : '' }}>
                        {{ $group->group_name }}
                    </option>
                @endif
            @endforeach
