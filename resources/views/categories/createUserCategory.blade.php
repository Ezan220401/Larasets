...
@section('titlePage', 'Tambah Kategori Pengguna')
...
@section('content')
                    ...
                    @if(auth()->user()->group_id == 2)
                        ...
                        <form action="{{ route('user_categories.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf

                            <div class="form-group">
                                <label for="group_name"><b>Nama Kategori</b></label>
                                <input type="text" class="form-control @error('group_name') is-invalid @enderror" 
                                    name="group_name" id="group_name" value="{{ old('group_name') }}" required>
                                <!-- error message -->
                                @error('group_name')
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('group_desc') is-invalid @enderror" 
                                    name="group_desc" id="group_desc" value="{{ old('group_desc') }}" required>
                                <!-- error message -->
                                @error('group_desc')
                                <div class="invalid-feedback">
                                    {{ $message }}
                        ...
                        @else
                            <div class="container mt-5 mb-5">
                                <div class="alert alert-danger" role="alert">
                                    Hanya Akademik yang dapat menambah kategori Pengguna.
                                </div>
                            </div>
                        @endif
    @endsection
